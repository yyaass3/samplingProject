...
for i in range(count):
    if Data[i][0] >= '2019-02-00' and Data[i][0] <= '2019-03-00':
        for j in data['NO2'].groupby(Data[i][1]['City']):
            no.append(j)
...
for i in range(count3):
    if Data2[i][0] >= '2019-04-00' and Data2[i][0] <= '2019-05-00':
        for j in data['NO2'].groupby(Data2[i][1]['City']):
            no2.append(j)
...
for i in range(count7):
    if Data3[i][0] >= '2019-07-00' and Data3[i][0] <= '2019-08-00':
        for j in data['NO2'].groupby(Data3[i][1]['City']):
            no3.append(j)
...
print(variance, ': مقدار وارانس میانگین برای شاخص 2NO')

''' run to text file '''
with open(r"F:\دانشگاه\نمونه گیری1\project\nemooneh2_project2.txt", 'w') as file:
    sys.stdout = file
    print('NO2')
